            ...
            <div class="flex justify-between">
                ...
                <div class="relative">
                    <router-link :to="'/contacts/' + contact.contact_id + '/edit'"
                                 class="px-4 py-2 rounded text-sm font-bold text-gray-500 mr-2 border border-green-500">
                        Edit
                    </router-link>
                    <a @click="modal = !modal"
                       class="px-4 py-2 border border-red-500 rounded text-sm font-bold text-red-500"
                       href="#">Delete</a>
                    <div v-if="modal" class="absolute bg-blue-900 text-white rounded-lg z-20 p-8 w-64 right-0 mt-2 mr-2">
                        <p>Are you sure you want to delete this record?</p>
                        <div class="flex items-center mt-6 justify-end">
                            <button class="text-white pr-4" @click="modal = !modal" >Cancel</button>
                            <button @click="destroy"
                                class="px-4 py-2 bg-red-500 rounded text-sm font-bold text-white">Delete</button>
                        </div>
                    </div>
